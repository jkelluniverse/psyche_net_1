...
- **[proposer-spec r3 · Claude M-5] The real hypothesis-matcher.** Exact-normalized-label matching makes ignition reachable, not likely — a blinded proposer only coincidentally emits the byte-exact becoming label. Design the semantic/evidence-overlap matcher (deterministic or human-confirmed; never an LLM truth gate) before belief-decay claims are made to paying practitioners at scale.
- **[proposer-spec r3 · D9, Jacob] Consent-scope UX + wound-reveal mediation.** `consentScope.betweenSessionExtraction` needs its named consent copy (not buried in general terms); the practitioner-mediated reveal flow for newly-extracted WOUND nodes (session-prep visibility → in-session or on-release client surfacing) is display-layer product work; both precede any real client. **(Extended, intake r2 · Claude E-2):** the consent copy must explicitly cover the PRE-SESSION intake moment — wedge clients complete intake *before session one*, arguably outside "between-session extraction"; the first extraction a client ever experiences must be one their consent line-item actually named.
- **[proposer-spec r3 · ChatGPT ENH#3] Third-party guard miss-rate metrics** over a larger adversarial phrase set ("the way my mother…" and friends), tracked longitudinally in the eval corpus.
- **[proposer-spec r3 · ChatGPT ENH#4] Retry/notice UX copy + crisis-bypass integration test** at the product layer (banner copy, timeouts, "view original entry" always available).
...
- **[renderer-lens r1 · D-1] Birth-data governance (LAW 8).** Birth data is quasi-special-category PII; `ChartImport.raw` echoes it. Before any real user: name the chart provider and its data-handling contract, define retention for `raw`, encryption-at-rest for the echoed birth fields, and verify the erasure path covers ChartImport + derived lens nodes + abstraction contributions. (The schema-blocking half — deletion cascade rule, re-import semantics, lensMapVersion rekey duty — is NOT deferred; it lives in renderer-lens spec v1.1's schema-deltas section.)
- **[renderer-lens r1 · ChatGPT E-16] Lens-mapping dry-run + unknown-feature telemetry.** A dry-run mode that outputs proposed ghosts without writes, plus counts of unknown/unused chart features — the tuning instrument for the lens map. Useful, not build-blocking.
- **[renderer-lens r1 · D-R5, Jacob] Veiled-WOUND pilot feedback item.** Veiled-but-present WOUND rendering is the v1 default (calm, forward-looking copy — "something to explore together next session" — never ominous; veiled nodes render singly, never as a clustered mass; crisis classification fully independent of the veil). If veiled presence proves anxiogenic in pilot, the fallback is practitioner-release-only visibility — a config change, not an architecture change. Watch for it explicitly in pilot feedback.
- **[eval autopsy · Jacob] Wrapper rejections persist the sanitized candidate.** Wrapper-stage `Proposal.payload` today carries only `{tempId, stage, detail}` — no label/type — which forced the miss autopsy into per-case attribution. Persist the sanitized canonical candidate shape (unknown fields stripped, per proposer spec §10.4) alongside the rejection so telemetry is per-expectation attributable. Build task, approved, not yet scheduled.
- **[renderer-lens r3 · A-5, Jacob] The WOUND release carrier (design sketch reserved).** v1 has NO release state: client-in-supervised is veiled ALWAYS, fail-closed. The per-node release carrier is bundle work with its projection seam reserved (`viewer.releasedNodeIds`); design sketch recorded verbatim from review: `NodeVisibilityRelease { id, nodeId, practitionerId, clientId, releasedAt, reason?, scope: 'LABEL'|'EVIDENCE' }`. Joins the existing D9 reveal-flow entry.
- **[renderer-lens r2 · Claude E-1] The static fringe ages — pilot watch.** The "edge of the unexplored" is an honest static affordance in v1; watch whether it still reads as a genuine boundary on mature maps (month-six skies), or has degraded into decoration. If the latter, it needs a computed input or removal — never a fake measurement.
- **[identity ruling · Jacob, 2026-07-15] 26% candidate-level paraphrase rate — named tuning target.** On the pre-v4 live diagnosis, 30 of 114 gate rejections were NO_VALID_EVIDENCE with per-evidence QUOTE_NOT_FOUND — the model paraphrasing instead of quoting, at candidate level ≈26%. Prompt v4 (held-theme labels + closest-known-key guidance) and the migration-9 rejection telemetry (`Proposal.rejectionDetail`) exist partly to move and MEASURE this number. Re-read it after v4 has real passes behind it; if it hasn't moved, the next lever is prompt-side quoting emphasis or adapter-side quote echo, never gate loosening.
- **[intake r1 · Claude M-6] STT provider data-handling line item** (provider, terms, region, retention) parallel to the LLM provider entry — the browser Web Speech API ships raw audio to the browser vendor's cloud, so the most sensitive words a person may ever speak transit a third party. LAW 8 work before any real user speaks; the intake spec's own copy now names the transit honestly (v1.1 §3).
- **[intake r1 · ChatGPT ENH-3] Deletion cascade covers intake artifacts:** intake SourceEvents + BECOMING seed declaration events/evidence named as sources in the LAW-8 erasure design; retraction-suite test noted for when that architecture lands.
- **[eval autopsy · Jacob] Corpus v2: label ALL entailing spans per expectation.** Entailment 0.373 on the v3 run is partly the scorer demanding the specific hand-labeled span when the model cited a different, genuinely-entailing quote — the metric is currently stricter than its name. Corpus v2 labels every entailing span per expectation (and the scorer docs say what the number means). Approved, not yet scheduled.
