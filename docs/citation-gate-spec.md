...
A node materializes (becomes a real graph node) only when it clears a conservative threshold. v1 rule: **≥ 2 distinct conferring evidence spans from ≥ 2 distinct source events** (recurrence, not a single mention). Valid-but-subthreshold proposals go to the `shadowBuffer` — retained and re-evaluated as new events arrive, never discarded (nothing is deleted). Hypothesis-provenance nodes (lens/becoming/practitioner) skip this and materialize at mass 0 in `HYPOTHESIS` state. **Unreachable-by-construction note (renderer-lens v1.3, C-7): the LENS arm of this rule is dead code in the live pipeline — `ExtractableNodeType` excludes LENS, the wrapper fails LENS proposals at shape validation in every mode, and all lens creation flows through the lens lane (renderer-lens spec §1). It stays specified for the gate's pure-function completeness, like the deprecated enum member — do NOT "fix" a rejected LENS proposal by widening the wrapper.**
...
base = 0.4
```
Hypothesis nodes start at a low confidence floor (config key `confidence.hypothesisFloor`, e.g. 0.15) and rise only as lived evidence confirms them. Confidence is ALWAYS written; there is no "unknown = null" — unknown is represented as an explicit low number and rendered as such (LAW 5).
