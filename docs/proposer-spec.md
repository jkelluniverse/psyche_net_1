...
*Psyche-Net · the adversarially-exposed front-end · v1.2 · governs `/src/engine/proposer/`*
...
| PRACTITIONER_SUPPORTED (verified) | SOLO set + WOUND |
...
- **The local shape validator always runs** (v1.2, C-5): provider-enforced structured output is an optimization, never the boundary — per-candidate validation is identical with or without it.
