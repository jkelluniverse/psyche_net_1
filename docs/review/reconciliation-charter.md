...
- **FINAL-drift amendment (adopted by Jacob at intake round 1; founding precedent: the IDEA-021 candidate-identity ruling shipping `heldShadowLabels` while proposer v1.4-FINAL stayed silent — caught by the intake round's cross-model lane):** the FINAL stamp protects *behavior*, not prose fossilization. Any ruling that changes a FINAL spec's described behavior triggers that spec's doc-sync **in the same integration commit** — a version bump whose changelog names the ruling as its source. A zero-behavior-change doc-sync carries the FINAL stamp forward and does not reopen review.
