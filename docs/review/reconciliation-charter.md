...
- **Tripwire amendment (armed by Jacob after renderer-lens round 3; auto-adopts on next occurrence, no further discussion):** the "rule-landed-test-didn't" class (a grammar/behavior rule integrated without its test carrier in the same edit) has occurred once post-warning. On its NEXT occurrence in any round, the following becomes a hard rule automatically: *"every grammar/behavior rule lands with its §-tests carrier in the same edit."*
